...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderCarVO updateOrderCar(Integer loginUserId,OrderCarDTO orderCarDTO) throws JsonProcessingException {
        Client client = clientService.selectClientByCId(loginUserId);
        ...
        if (orderCarVO.getType() == 1){//优惠券订单
            ...
            orderCarVO.getOrderDetails().get(1).setCover(coupon.getCover());//优惠券图片
            orderCarVO.getOrderDetails().get(1).setGoodsName(coupon.getName());//优惠券名称
            ...
            if(orderCarVO.getActivityId() != null){//参与了营销活动
                ...
                if (activity.getVip() && !client.getVip()){throw new BusinessException("该营销活动仅限VIP用户");}
        ...
        //存入redis
        String orderCarDTOJson = objectMapper.writeValueAsString(orderCarDTO);
        stringRedisTemplate.opsForValue().set("sys:goods:car:user:"+loginUserId,orderCarDTOJson,10, TimeUnit.HOURS);
        return orderCarVO;
    }

    @Override
    public OrderCarVO getOrderCar(Integer loginUserId) throws JsonProcessingException {
        String orderCarVOJson = stringRedisTemplate.opsForValue().get("sys:goods:car:user:" + loginUserId);
    ...
    public Integer insertOrder(Integer loginUserId) throws JsonProcessingException {
        ...
        OrderExDTO orderExDTO = objectMapper.readValue(orderCarJson, OrderExDTO.class);
        //构建订单
        orderExDTO.setNo(uniqueNo.getUniqueNo("ORD"));//设置订单编号
        orderExDTO.setClientType(0);//设置客户类型为系统客户
        ...
        List<OrderExGoodsConfigDTO> orderExDetailConfigList = new ArrayList<>();//菜品明细中的配置项
        List<OrderExComboDetailDTO> comboDetailList = new ArrayList<>();//套餐明细中的菜品
        ...
        MaterialUsage.forEach((materialId,num) -> inventoryExService.updateRepertory(2,orderExDTO.getId(),storeIds.get(0),materialId,1,num));
        
        //清空购物车
        stringRedisTemplate.delete("sys:goods:car:user:" + loginUserId);
        //发送订单延迟消息
        rabbitTemplate.convertAndSend("order_delay_exchange","order_delay_key",orderExDTO.getId(),msg->{
            msg.getMessageProperties().setExpiration("360000");//设置消息延迟时间为6分钟
            return msg;
        });
    ...
    public OrderPayVO payOrder(OrderPayDTO orderPayDTO) {
        ...
        //TODO编写支付相关逻辑(后台端,生成二维码或扫码交易)

        //TODO测试代码,测试支付成功,后续移到支付结果回调接口
        order.setState(2);//设置订单状态为制作中
        order.setPayTime(LocalDateTime.now());//设置订单支付时间
        order.setPickupNum(uniqueNo.getTackNo(order.getDeptId(),order.getEatMode()));
        orderMapper.updateOrder(order);//修改订单
        return null;
    }

    @Override
    public Integer selectOrderState(Integer id) {
        OrderVO orderVO = orderMapper.selectOrderById(id);
        return orderVO.getState();
    }

    @Override
    public List<OrderDetail> selectOrderBaseDetailByOIds(List<Integer> ids) {
        return orderMapper.selectOrderBaseDetailByOIds(ids);
    }

    @Override
    public List<Order> selectPayTimeOutOrder() {
        LocalDateTime now = LocalDateTime.now();
        return orderMapper.selectPayTimeOutOrder(now);
    }

    @Override
    public OrderVO selectOrderById(Integer loginUserId,Integer dataCoverage, Integer id) {//获取订单基本信息
        OrderVO orderVO = orderMapper.selectOrderById(id);
    ...
    @Override
    public Order selectOrderBaseById(Integer id) {
        return orderMapper.getBaseOrderByOId(id);
    }

    @Override
    public PageResult<OrderSimpleVO> selectOrderList(Integer loginUserId, Integer dataCoverage, Integer page, Integer pageSize, String no, Integer type, String dept, Integer state, Integer eatMode) {
        List<Integer> validDeptIds = null;
        switch (dataCoverage){
            case 2:
                validDeptIds = deptService.getDeptIdsByUserId(loginUserId);
                break;
            case 1:
                validDeptIds = deptService.getAllDeptIdsByUserId(loginUserId);
                break;
            case 0:
                break;
        }
        Integer total = orderMapper.countOrderList(no,type,dept,state,eatMode,validDeptIds);
        Integer skip = (page-1)*pageSize;
        //获取订单基本信息
        List<OrderSimpleVO> orderBaseList = orderMapper.selectOrderList(skip,pageSize,no,type,dept,state,eatMode,validDeptIds);
        ...
        List<OrderTDetailDTO> orderTDetails = orderMapper.batchSelectOrderDetailByOId(orderIds);
        ...
        //根据菜品明细id列表获取其配置信息
        List<OrderTDConfigDTO> oDConfigs = orderMapper.selectODConfigByDIds(dishDetailIds);
        //根据套餐明细id列表获取其菜品信息
        List<OrderTCDetailDTO> oDCDetail = orderMapper.selectOCDetailByDIds(comboDetailIds);
        //收集套餐明细中的唯一id列表
        List<Integer> comboInnDetailIds = oDCDetail.stream().map(OrderTCDetailDTO::getId).toList();
        //根据套餐中的唯一id列表获取其菜品信息
        List<OrderTCDConfigDTO> oCDConfigs = orderMapper.selectOCDConfigByDIds(comboInnDetailIds);

        //        for (OrderSimpleVO orderSimpleVO : orderBaseList) {
//            List<OrderDetailVO> detailList = orderTDetails.stream()
//                    .filter(detail -> detail.getOrderId().equals(orderSimpleVO.getId()))
//                    .map(detail -> {
//                        OrderDetailVO orderDetailVO = new OrderDetailVO();
//                        BeanUtils.copyProperties(detail, orderDetailVO);
//                        orderDetailVO.setId(detail.getDetailId());
//                        if (orderDetailVO.getGoodsType() == 0){//该条订单明细是菜品
//                            List<OrderGoodsConfigVO> configList = oDConfigs.stream()
//                                    .filter(config -> config.getDetailId().equals(orderDetailVO.getId()))
//                                    .map(config -> {
//                                        OrderGoodsConfigVO configVO = new OrderGoodsConfigVO();
//                                        configVO.setValueId(config.getValueId());
//                                        configVO.setName(config.getName());
//                                        return configVO;
//                                    }).toList();
//                            orderDetailVO.setConfigs(configList);
//                        }else if(orderDetailVO.getGoodsType() == 1){//该条订单明细是套餐
//                            List<OrderComboDetailVO> dishList = oDCDetail.stream()
//                                    .filter(dish -> dish.getDetailID().equals(orderDetailVO.getId()))
//                                    .map(dish -> {
//                                        OrderComboDetailVO dishVO = new OrderComboDetailVO();
//                                        dishVO.setDishId(dish.getDishId());
//                                        dishVO.setDishName(dish.getDishName());
//                                        dishVO.setNum(dish.getNum());
//                                        List<OrderGoodsConfigVO> configList = oCDConfigs.stream()
//                                                .filter(config -> config.getComboDetailId().equals(dish.getId()))
//                                                .map(config -> {
//                                                    OrderGoodsConfigVO orderGoodsConfigVO = new OrderGoodsConfigVO();
//                                                    orderGoodsConfigVO.setName(config.getName());
//                                                    orderGoodsConfigVO.setValueId(config.getValueId());
//                                                    return orderGoodsConfigVO;
//                                                }).toList();
//                                        dishVO.setConfigs(configList);
//                                        return dishVO;
//                                    }).toList();
//                            orderDetailVO.setComboDetails(dishList);
//                        }
//                        return orderDetailVO;
//                    }).toList();
//            orderSimpleVO.setOrderDetails(detailList);
//        }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateOrderState(Integer loginUserId, Integer dataCoverage, OrderStateDTO orderStateDTO) {
        Order order = orderMapper.getBaseOrderByOId(orderStateDTO.getId());//获取订单
        if (order == null){throw new BusinessException("订单不存在");}
        switch (dataCoverage){
            case 2:
                if (!Objects.equals(order.getClientId(), loginUserId)){throw new AuthorityException("权限不足");}
                break;
            case 1:
                if (!deptService.getAllDeptIdsByUserId(loginUserId).contains(order.getDeptId())){
        ...
        switch (orderStateDTO.getState()){
            ...
            case 5://申请退款
                ...
                if (orderStateDTO.getCause().isEmpty()){throw new BusinessException("请填写申请退款原因");}
                order.setState(5);
                orderMapper.updateOrder(order);//修改订单
                break;
            default:
    ...
    @Override
    public void batchCancelOrder(List<Integer> ids) {
        orderMapper.batchCancelOrder(ids);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void refundOrder(Integer loginUserId,Integer dataCoverage,OrderRefundDTO orderRefundDTO) {
        Order order = orderMapper.getBaseOrderByOId(orderRefundDTO.getId());//获取订单基本信息
        if (order == null){throw new BusinessException("订单不存在");}
        if(order.getState()==0 || order.getState()==1){throw new BusinessException("订单未支付");}
        if (order.getClientType() == 1 && order.getState()!=5){throw new BusinessException("订单未申请退款");}
        switch (dataCoverage){
            case 2:
                if (!deptService.getDeptIdsByUserId(loginUserId).contains(order.getDeptId())){throw new AuthorityException("权限不足");}
                break;
            case 1:
                if (!deptService.getAllDeptIdsByUserId(loginUserId).contains(order.getDeptId())){throw new AuthorityException("权限不足");}
                break;
            case 0:
                break;
        }
        ...
        switch (order.getType()){
            case 0://点餐订单
                if (!orderRefundDTO.getMakeStart()){//订单未开始制作
                    //原料库存回滚
                    inventoryExService.rollbackRepertory(2,List.of(order.getId()));
                }
                //优惠券回滚
                this.rollbackOrderCoupon(List.of(order.getId()));
            case 1://优惠券订单
                //优惠券回滚到库存中
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void rollbackOrderCoupon(List<Integer> orderIds) {
//        Order order = orderMapper.getBaseOrderByOId(orderId);//获取订单
        List<OrderCouRollbackDTO> clientCouponList = orderMapper.batchSelectOrderCouponByOIds(orderIds);//批量获取订单中的优惠券与顾客关系
//        List<OrderCouponVO> orderCouponList = orderMapper.selectOrderCouponByOId(orderId);//获取本单使用的优惠券
        if (!clientCouponList.isEmpty()){//存在需要回滚的优惠券
            List<MarketingCUseNum> couponUseNumList = clientCouponList
                    .stream()
                    .map(clientCoupon->new MarketingCUseNum(clientCoupon.getCouponId(), clientCoupon.getNum()))
                    .toList();
//            Map<Integer,Integer> couponUseNumMap = new HashMap<>();
            marketingService.batchInsertCouponToClient(clientCouponList);//批量添加优惠券到用户券包
//            for (OrderCouponVO orderCoupon : orderCouponList) {
//                //统计订单中每种优惠券数量
//                couponUseNumMap.put(orderCoupon.getId(), couponUseNumMap.getOrDefault(orderCoupon.getId(), 0) + 1);
//            }
//            for (Map.Entry<Integer, Integer> entry : couponUseNumMap.entrySet()){
//                MarketingCUseNum marketingCUseNum = new MarketingCUseNum(entry.getKey(), entry.getValue()*-1);
//                couponUseNumList.add(marketingCUseNum);
//                //返还优惠券给顾客
//                marketingService.insertCouponToClient(entry.getKey(), order.getClientId(), entry.getValue());//添加优惠券到用户券包
//            }
            //优惠券核销数量回滚
